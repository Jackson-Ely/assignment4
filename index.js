...
async function connectDB() {
    try {
        ...
        booksCollection = db.collection("bookcollection");
...
const server = http.createServer(async (req, res) => {
    ...
    if (pathname === "/login" && req.method === "GET") {
        ...
        if (session) {
            console.log("[ROUTE] Already logged in, redirecting to /books");
    ...
    if (pathname === "/") {
        if (session) {
            console.log("[ROUTE] / -> redirecting to /books (logged in)");
            res.writeHead(302, { Location: "/books" });
        } else {
    ...
    if (pathname === "/api" && req.method === "GET") {
        ...
        booksCollection
            ...
            .catch((err) => {
                console.log("[API] ERROR fetching books:", err.message);
                sendJSON(res, 500, { error: "Failed to fetch books" });
            });
        return;
    }
    ...
    if (pathname === "/api" && req.method === "POST") {
        ...
        book.addedBy = session.username;
        console.log(`[API] Adding book: "${book.title}" by "${book.author}" (user: ${session.username})`);
        booksCollection
            .insertOne(book)
            .then((result) => {
                console.log("[API] Book inserted:", result.insertedId);
                sendJSON(res, 201, result);
            })
            .catch((err) => {
                console.log("[API] ERROR inserting book:", err.message);
                sendJSON(res, 500, { error: "Failed to add book" });
            });
        return;
    }
    ...
    if (pathname.startsWith("/api/") && req.method === "PUT") {
        ...
        console.log(`[API] PUT update book _id=${id}`);
        const body = await readBody(req);
        let updates;
        try {
        ...
        booksCollection
            .updateOne({ _id: oid }, { $set: updates })
            .then((result) => {
                console.log(`[API] Updated: matchedCount=${result.matchedCount}, modifiedCount=${result.modifiedCount}`);
                sendJSON(res, 200, result);
            })
            .catch((err) => {
                console.log("[API] ERROR updating book:", err.message);
                sendJSON(res, 500, { error: "Failed to update book" });
            });
        return;
    }
    ...
    if (pathname.startsWith("/api/") && req.method === "DELETE") {
        ...
        console.log(`[API] DELETE book _id=${id}`);
        booksCollection
            .deleteOne({ _id: oid })
            .then((result) => {
            ...
            .catch((err) => {
                console.log("[API] ERROR deleting book:", err.message);
...
connectDB().then(() => {
    server.listen(PORT, () => {
        ...
        console.log("  GET  /books  - book collection page (protected)");
        console.log("  GET  /about  - about page (protected)");
        console.log("  GET  /api    - fetch all books (protected)");
        console.log("  POST /api    - add book (protected)");
        console.log("  PUT  /api/:id   - update book (protected)");
        console.log("  DELETE /api/:id - delete book (protected)");
